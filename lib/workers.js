...
class ImpressWorker {
  constructor(env) {
    ...
    };
    {
      ...
      const debugPortOffset = env.WORKER_ID;

      if (opt.execArgv.some(arg => debugArgRegex.test(arg))) {
        let inspectPort = process.debugPort + debugPortOffset;
        if (inspectPort > maxPort) {
          inspectPort -= maxPort - minPort + 1;
        }

        opt.execArgv.push(`--inspect-port=${inspectPort}`);
      }
    }
